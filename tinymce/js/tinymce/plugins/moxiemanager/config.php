<?php
	...
	session_start();
	if (isset( $_SESSION["UsrPriv"]) &&  $_SESSION["UsrPriv"]>=90){
		$root =  $rootpath. '/uploadedImages/';
	} elseif (isset( $_SESSION["UsrPriv"]) && $_SESSION["UsrPriv"]<90) {
		$root =  $rootpath. '/uploadedImages/Users/'.$_SESSION["FName"]."_".$_SESSION["UsrID"]."/";
	}else {
		$root =  $rootpath. '/uploadedImages/';
	}
	// General
	...
	// Filesystem
	$moxieManagerConfig['filesystem.rootpath'] = $root;
	$moxieManagerConfig['filesystem.include_directory_pattern'] = '';
	$moxieManagerConfig['filesystem.exclude_directory_pattern'] = '/^(mcith|Temp)$/i';
