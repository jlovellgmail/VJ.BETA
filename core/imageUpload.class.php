...
class imageUpload extends upload {
   ...
   function cropInspSquareImage($id, $x, $y, $x1, $y1, $isAdmin, $FName) {
    ...
    private function save() {
        ...
        if ($this->processed) {
        		$PathToReturn = $this->file_dst_pathname;
        		$rootp = $_SERVER['DOCUMENT_ROOT'];
        		$index = strpos($PathToReturn, "\uploadedImages");
        		$path = substr($PathToReturn, $index);
        		copy(str_replace('\\', '/', $rootp.$path), '/\\SNPV1\MJC\VirgilJames' . $path);
