...
$inspirations = $Ambassador->getInspirations();
...
<div class="widthWrapper">
    ...
    <div class="sm-tweleve row textLeft marTop30">
     ...
     --><div class="sm-twelve md-four lg-four textRight marTop15 padTop6">
            <a class="textBtn size6" href="Ambassador/ambInspirationCreate.php">Add Item</a>
        </div>
    </div>  
        ...
        <div class='grid' style='margin: -4px;'>
            <?php
                foreach ($inspirations as $inspiration) {
                    $IID = $inspiration->getVar("IID");
                    $imageTitle = $inspiration->getVar("ImageTitle");
                    $imgUrl = $inspiration->getImgUrl();
                    $imgType = $inspiration->getVar("ImgType");
                    $cssWrapper = 'packery-gallery-wrapper';
                    $cssSizing = 'packery-gallery-sizing';
                    $cssContent = 'packery-gallery-content';
                    if ($imgType == 'W')
                    {
                    		$cssWrapper = 'packery-post-landscape-wrapper';
                    		$cssSizing = 'packery-post-landscape-sizing';
                    		$cssContent = 'packery-post-landscape-content';
                    }
                    else if ($imgType == 'T')
                    {
                    		$cssWrapper = 'packery-post-portrait-wrapper';
                    		$cssSizing = 'packery-post-portrait-sizing';
                    		$cssContent = 'packery-post-portrait-content';
                    }
                    ?><div class='grid-item <?php echo $cssWrapper; ?>'>
                			<div class='<?php echo $cssSizing; ?>'></div>
                			<div class='grid-item-content <?php echo $cssContent; ?>' style='background: url(<?php echo $imgUrl; ?>) no-repeat center; background-size:cover;'>
                    			<div class='grid-item-overlay'>
                        			<div class="title-wrapper tableWrapper">
                            			<div class="cellWrapper">
                                			<span class="gallery-img-title size5" title="hello"><?php echo $imageTitle; ?></span>
                            			</div>
                        			</div>
                        			<div class="gallery-img-button-wrapper">
                            			<a class="gallery-img-view size7" href="javascript:void(0);" onclick="showModal('/incs/modals/inspirationModal.php?IID=<?php echo $IID; ?>');">View</a><!-- 
                         			--><a class="gallery-img-edit size7" href="/Ambassador/ambInspirationCreate.php?IID=<?php echo $IID; ?>">Edit</a>
                        			</div>
                    			</div>
                			</div>
            			</div><?php
